    ...
    <div className="relative overflow-hidden border-b border-border min-h-[400px] md:min-h-[520px]">
      ...
      >
        ...
        <p className="text-[13px] md:text-[15px] text-cream-secondary max-w-[440px] leading-relaxed font-light mb-6 md:mb-7">
          Every tactic has a measured win rate, structured reviews with match data,<br className="hidden md:inline" /> and context by version, level, and style.
        </p>
        <div className="flex items-center gap-3 md:gap-4">
          <button
            onClick={() => document.getElementById('revamp-explore')?.scrollIntoView({ behavior: 'smooth' })}
          ...
          <a href="#revamp-social" className="font-mono text-[11px] md:text-[12px] text-cream-muted transition-colors duration-300 hover:text-copper">
            See top creators →
          </a>
        </div>

        {/* Community proof bar */}
        <div className="flex items-center gap-3 md:gap-5 mt-6 md:mt-8">
          <span className="font-mono text-[10px] md:text-[11px] text-cream-faint tracking-[0.02em]">8,400+ managers</span>
          <span className="w-[3px] h-[3px] rounded-full bg-cream-faint opacity-50" />
          <span className="font-mono text-[10px] md:text-[11px] text-cream-faint tracking-[0.02em]">2,100+ rated tactics</span>
          <span className="w-[3px] h-[3px] rounded-full bg-cream-faint opacity-50" />
          <span className="font-mono text-[10px] md:text-[11px] text-cream-faint tracking-[0.02em] hidden sm:inline">140k+ matches tracked</span>
        </div>
      </div>

      {/* Tactic name - bottom right */}
