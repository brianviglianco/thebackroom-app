...
export default function HeroRefinedCTA() {
  ...
  return (
    <div className="relative overflow-hidden border-b border-border min-h-[400px] md:min-h-[520px]">
      <div className="absolute inset-0" style={{ background: 'linear-gradient(165deg, var(--bg) 0%, #1C2218 30%, #20291D 60%, #263524 100%)' }} />
      <div className="hidden md:block">
        <HeroCanvas onActionChange={handleActionChange} />
      </div>
      <div className="absolute inset-0 z-[2]" style={{ background: 'linear-gradient(to right, var(--bg) 0%, rgba(26,23,20,0.97) 12%, rgba(26,23,20,0.85) 28%, rgba(26,23,20,0.55) 45%, rgba(26,23,20,0.2) 62%, rgba(26,23,20,0) 80%)' }} />
      <div className="absolute top-0 left-0 right-0 h-[90px] z-[3]" style={{ background: 'linear-gradient(to bottom, rgba(26,23,20,0.9) 0%, rgba(26,23,20,0.4) 50%, rgba(26,23,20,0) 100%)' }} />
      <div className="absolute bottom-0 left-0 right-0 h-[60px] z-[3]" style={{ background: 'linear-gradient(to top, var(--bg), rgba(26,23,20,0))' }} />
      ...
      <div className="hidden md:flex absolute bottom-8 right-9 z-[6]">
        <div className={`font-mono text-[14px] tracking-[0.2em] uppercase text-copper text-right transition-opacity duration-700 ${actionFeed.name ? 'opacity-80' : 'opacity-0'}`}>
          {actionFeed.name}
        </div>
      </div>
    </div>
